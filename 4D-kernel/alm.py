...
def orthogonalityCheck(matrix, batch_Size):
    constraint_vector = torch.zeros(batch_Size, (int)(NumberOfEigenFun)*(NumberOfEigenFun-1), dtype=torch.float)
    sumVal = 0
    t = 0
    for i in range(batch_Size):
        for j in range(NumberOfEigenFun):
            for k in range(NumberOfEigenFun-(j+1)):
                ...
                constraint_vector[i,t] = constraint.real
                constraint_vector[i,t+1] = constraint.imag
                t = t + 2
        t =0
...
class CustomLoss(nn.Module):
    ...
    def forward(self, nnOutput, train_kernals_2d, batch_Size, batchSize_Sqrt, penalty, lambda_1, lambda_2):
        ...
        constraint_vector1_avg = constraint_vector1_avg.reshape((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1)
        constraint_vector2_avg = constraint_vector2_avg.reshape((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1)
        ...
        return loss, mse_norm , constraint1_abs_sum, constraint2_abs_sum, constraint_vector1_avg, constraint_vector2_avg, constraints_L2_norm
...
##lambda1VLast = torch.zeros(((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1))
##lambda2VLast = torch.zeros(((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1))
...
def trainNN(train_kernel_split, train_kernel, test_kernel_split, test_kernel):
    ...
    lambda1V = torch.full(((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1), 0.0)
    lambda2V = torch.full(((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1), 0.0)
    penalty = 1
    alpha = 1.01
    #alpha = 1.01

    constraint_vector1_avg = torch.zeros((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1, dtype=torch.float)
    constraint_vector2_avg = torch.zeros((int)(NumberOfEigenFun)*(NumberOfEigenFun-1),1, dtype=torch.float)
...
#        print(f'Epoch {t+1}')
        ...
        for b in range(NumberOfTrainBatches):
            ...
            loss ,mse_norm, constraint1_abs_sum, constraint2_abs_sum, constraint_vector1_avg, constraint_vector2_avg, constraints_L2_norm = custom_loss(nnOut, train_kernel[(b*BatchSize):(b+1)*BatchSize, :, :], BatchSize, BatchSize_Sqrt, penalty, lambda1V, lambda2V)
...
##                lambda2V = lambda2V + penalty *  constraint_vector2_avg
        ...
        with torch.no_grad():
            for v in range(NumberOfTestBatches):
                ...
                loss ,mse_norm, constraint1_abs_sum, constraint2_abs_sum, constraint_vector1_avg_test, constraint_vector2_avg_test, constraints_L2_norm_test = custom_loss(nnOut, test_kernel[(v*BatchSize):(v+1)*BatchSize, :, :], BatchSize, BatchSize_Sqrt, penalty, lambda1V, lambda2V)
        ...
        if (t+1)%25 == 0:
            ...
            sio.savemat('N10.mat', data_to_save)
            torch.save(model.state_dict(), 'N10.pth')

##        if(t>50):
##            if(loss < 0.5):
##                if(torch.sum(train_obj1_train[t-50:t])/(50*(NumberOfEigenFun*(NumberOfEigenFun-1))) < 0.0001) and (torch.sum(train_obj2_train[t-50:t])/(50*(NumberOfEigenFun*(NumberOfEigenFun-1))) < 0.0001):
##                    print(t)
##                    epochs = t
##                    break
...
##
##model2 = NNetwork()
##model2.load_state_dict(torch.load('N10.pth'))
##model2.eval()
